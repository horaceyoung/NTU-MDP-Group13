...
public class
BtPopUp extends AppCompatActivity {
    private static final String TAG = "BtPopUp";
    ...
    BtConnectionSvc mBluetoothConnection;
    ...
    Runnable reconnectionRunnable = new Runnable() {
        ...
        public void run() {
            // Magic here
            try {
                if (BtConnectionSvc.BluetoothConnectionStatus == false) {
                    startBTConnection(mBTDevice, myUUID);
                    Toast.makeText(BtPopUp.this, "Reconnection Success", Toast.LENGTH_SHORT).show();

                }
                reconnectionHandler.removeCallbacks(reconnectionRunnable);
                retryConnection = false;
            } catch (Exception e) {
                Toast.makeText(BtPopUp.this, "Failed to reconnect, trying in 5 second", Toast.LENGTH_SHORT).show();
            }
        }
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.bt_pop_up_window);
...
//Display a list of BT devices

        otherDevicesLV = (ListView) findViewById(R.id.otherDevicesLV);
        pairedDevicesLV = (ListView) findViewById(R.id.pairedDevicesLV);
...
//To select other BT device to connect.
        otherDevicesLV.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                mBluetoothAdapter.cancelDiscovery();
                pairedDevicesLV.setAdapter(mPairedDevlceListAdapter);
                ...
                if (Build.VERSION.SDK_INT > Build.VERSION_CODES.JELLY_BEAN_MR2) {
                    ...
                    mBluetoothConnection = new BtConnectionSvc(BtPopUp.this);
                    mBTDevice = mNewBTDevices.get(i);
                }
            }
        });
//To select paired BT device to connect.
        pairedDevicesLV.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                mBluetoothAdapter.cancelDiscovery();
                otherDevicesLV.setAdapter(mNewDevlceListAdapter);
        ...
        bluetoothSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton compoundButton, boolean isChecked) {
                ...
                if(mBluetoothAdapter ==null){
                    ...
                    Toast.makeText(BtPopUp.this, "Device Does Not Support Bluetooth capabilities!", Toast.LENGTH_LONG).show();
...
// When select on connect button, it will start to connect with the selected device
        connectBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(mBTDevice ==null)
                {
                    Toast.makeText(BtPopUp.this, "Please Select a Device before connecting.", Toast.LENGTH_LONG).show();
                }
                else {
                    startConnection();
                }
        ...
        connStatusTV = (TextView) findViewById(R.id.connStatusTV);
        connStatus ="Disconnected";
        sharedPreferences = getApplicationContext().getSharedPreferences("Shared Preferences", Context.MODE_PRIVATE);
        if (sharedPreferences.contains("connStatus"))
            connStatus = sharedPreferences.getString("connStatus", "");

        connStatusTV.setText(connStatus);

        backBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                editor = sharedPreferences.edit();
                editor.putString("connStatus", connStatusTV.getText().toString());
                editor.commit();
                finish();
            }
        });

        myDialog = new ProgressDialog(BtPopUp.this);
    ...
    public void toggleButtonScanBt(View view){
        Log.d(TAG, "toggleButton: Scanning for unpaired devices.");
        mNewBTDevices.clear();
        if(mBluetoothAdapter != null) {
            if (!mBluetoothAdapter.isEnabled()) {
                Toast.makeText(BtPopUp.this, "Please turn on Bluetooth first!", Toast.LENGTH_SHORT).show();
            }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if(action.equals(BluetoothDevice.ACTION_BOND_STATE_CHANGED)){
                ...
                if(mDevice.getBondState() == BluetoothDevice.BOND_BONDED){
                    ...
                    Toast.makeText(BtPopUp.this, "Successfully paired with " + mDevice.getName(), Toast.LENGTH_SHORT).show();
                    mBTDevice = mDevice;
                }
    ...
    private BroadcastReceiver mBroadcastReceiver5 = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if(status.equals("connected")){
                ...
                Toast.makeText(BtPopUp.this, "Device now connected to "+mDevice.getName(), Toast.LENGTH_LONG).show();
                editor.putString("connStatus", "Connected to " + mDevice.getName());
                connStatusTV.setText("Connected to " + mDevice.getName());
            }
            else if(status.equals("disconnected") && retryConnection == false){
                Log.d(TAG, "mBroadcastReceiver5: Disconnected from "+mDevice.getName());
                Toast.makeText(BtPopUp.this, "Disconnected from "+mDevice.getName(), Toast.LENGTH_LONG).show();
                mBluetoothConnection = new BtConnectionSvc(BtPopUp.this);
//                mBluetoothConnection.startAcceptThread();
                ...
                TextView connStatusTV = findViewById(R.id.connStatusTV);
                connStatusTV.setText("Disconnected");
                editor.commit();

                try {
                    myDialog.show();
                }catch (Exception e){
                    Log.d(TAG, "BtPopUp: mBroadcastReceiver5 Dialog show failure");
                }

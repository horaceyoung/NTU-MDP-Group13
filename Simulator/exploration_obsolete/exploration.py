...
class Exploration:
    ...
    def runExploration(self):
        ...
        self.senseAndReload()
        self.areaExplored = (
            self.calculateAreaExplored()
        )  # Number of cells explored in grid
    ...
    def explorationLoop(self, r, c):
        while True:
            ...
            if self.areaExplored >= self.coverageLimit or time.time() >= self.endTime:
                print(
                    "areaExplored, coverageLimit, curTime, endTime: ",
                    self.areaExplored,
                    self.coverageLimit,
                    time.time(),
                    self.endTime,
    ...
    def northFree(self):
        ...
        return (
            self.isExploredNotObstacle(botRow + 1, botCol - 1)
            and self.isExploredAndFree(botRow + 1, botCol)
            and self.isExploredNotObstacle(botRow + 1, botCol + 1)
        )
    ...
    def eastFree(self):
        ...
        return (
            self.isExploredNotObstacle(botRow - 1, botCol + 1)
            and self.isExploredAndFree(botRow, botCol + 1)
            and self.isExploredNotObstacle(botRow + 1, botCol + 1)
        )

...
class Map:
    ...
    def __init__(self,bot):
        ...
        for x in range(configurations.MAP_ROWS):
            for y in range(configurations.MAP_COLS):
                ...
                if(x==0 or y==0 or x==(configurations.MAP_ROWS-1) or  y==(configurations.MAP_COLS-1)):
    ...
    def checkValidCoordinates(self,row,col):
        return (row >= 0 and col >= 0 and row < configurations.MAP_ROWS and col < configurations.MAP_COLS)
    ...
    def inGoalZone(self,row,col):
        return (row <= configurations.GOAL_ROW + 1 and row >= configurations.GOAL_ROW - 1 and col <= configurations.GOAL_COL + 1 and col >= configurations.GOAL_COL - 1)
    ...
    def setObstacleCell(self,row,col,obstacle):
        ...
        if (row >= 1):
            ...
            if (col < configurations.MAP_COLS - 1):
                self.grid[row - 1][col + 1].setVirtualWall(obstacle)    #bottom-right cell
    ...
    def getIsObstacleOrWall(self,row,col):
        return (not self.checkValidCoordinates(row, col)) or self.getCell(row, col).isObstacle
...
if __name__ == '__main__':
    test_bot = robot.RobotAlgo(configurations.START_ROW,configurations.START_COL,False)

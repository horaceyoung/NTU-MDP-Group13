...
while running:
    ...
    for event in pg.event.get():
        ...
        if event.type == pg.KEYDOWN:
            if event.key == pg.K_w:
                fastest_path.astar(arena_map,player_robot, (18, 1), (1, 13))
                #exploration.Exploration.next_move(player_robot, arena_map)
            if event.key == pg.K_e:
                ...
                #player_robot.rotate(-90)
            if event.key == pg.K_h:
                fastest_path.astar(arena_map,player_robot,player_robot.location,(18,1))
            ...
            if event.key == pg.K_r:
                ...
                if(start_exploration == "ES"):
                    exploration_instance = exploration.Exploration(300, 100, player_robot, arena_map, True, comm)
                    exploration_instance.initialize_exploration()
                    fastest_path.astar(arena_map,player_robot,player_robot.location,(18,1))

                start_fastest = comm.get_android_command()
                if(start_fastest == "FS"):
                    #fastest_path.astar(arena_map, (18, 1), (1, 13))
                    fastest_path.astar(arena_map,player_robot,player_robot.location,(18,1))
            if event.key == pg.K_t:
                
                comm.send_command("AL")
                comm.send()
                #comm.send_movement(Movement.FORWARD,Movement.FORWARD.value,False)

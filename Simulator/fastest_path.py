    ...
    # for i in range(0,1000):
        ...
        if current_node == end_node:
            ...
            if comm == None:
                ...
                comm.send_movement_calibrate()
                for route in waze:
                    if route[0] == "STRAIGHT":
                        if route[1] > 9:
                            comm.send_movement_forward(step=9, calibrate=False)
                            comm.send_movement_forward(step=route[1]-9, calibrate=False)
                        else:
                            comm.send_movement_forward(step=route[1], calibrate=False)
                        for i in range(route[1]):
                            robot.move_forward()
                    elif route[0] == "LEFT":
                        comm.send_movement_rotate_left(calibrate=False)
                        robot.rotate(-90)
                    elif route[0] == "RIGHT":
                        comm.send_movement_rotate_right(calibrate=False)
                        robot.rotate(90)

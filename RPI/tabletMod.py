    ...
    def connect_tablet(self):
        while True:
            ...
            try:
                ...
                print(
                    "Waiting for Bluetooth connection on RFCOMM channel %d.."
                    % self.port
                )
                (self.client_sock, addr) = self.server_sock.accept()
                cprint(
                    BOLD + GREEN,
                    "Successfully connected to Tablet! Tablet Bluetooth Address: "
                    + str(addr),
                )
    ...
    def read_tablet(self):
        try:
            ...
            if "Connection reset by peer" in str(error):
                ...
                cprint(
                    BOLD + RED,
                    "Bluetooth disconnected.. Tablet read failed.. Retrying bluetooth connection..",
                )
                self.connect_tablet()
